...
export function validateDataset(ds: Dataset): ValidationResult {
  ...
  for (const s of ds.species) {
    if (!hasName(s.commonName) || !hasName(s.scientificName)) {
      const label =
        s.scientificName ||
        (typeof s.commonName === 'string' ? s.commonName : s.commonName?.es) ||
        '?'
    ...
    for (const [tid, values] of Object.entries(s.traits)) {
      ...
      if (!trait) {
        errors.push(`species ${s.scientificName} references unknown trait ${tid}`)
        continue
      }
    ...
    if (needsNa && !(isNa(s.traits.margin) && isNa(s.traits.leafShape)))
      errors.push(
        `species ${s.scientificName}: needle/scale/palm must use margin ['n-a'] and leafShape ['n-a']`,
      )
  }
