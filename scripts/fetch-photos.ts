...
async function main() {
  ...
  const existing = new Set(
    (existsSync(OUT_DIR) ? await readdir(OUT_DIR) : []).map((f) => f.replace(/\.webp$/, '')),
  )
  ...
  for (const sp of targets) {
    ...
    try {
      ...
      await sleep(200)
      if (!imgFile) {
        discarded.push(`${sp.scientificName}: no P18`)
        continue
      }
      ...
      await sleep(200)
      if (!info) {
        discarded.push(`${sp.scientificName}: no commons info`)
        continue
      }
      if (!REUSABLE.test(info.license)) {
        discarded.push(`${sp.scientificName}: license "${info.license}"`)
        continue
      }
      ...
      if (FORCE || !existing.has(slug)) {
        ...
        if (!bin.ok) {
          discarded.push(`${sp.scientificName}: download ${bin.status}`)
          continue
        }
        ...
        console.log(
          `ok  ${sp.scientificName.padEnd(28)} ${(webp.length / 1024).toFixed(0)}KB  ${info.license}`,
        )
        await sleep(200)
      } else {
  ...
  const sorted = Object.keys(manifest)
    .sort()
    .reduce<Record<string, PhotoMeta>>((a, k) => ((a[k] = manifest[k]!), a), {})
  const body =
