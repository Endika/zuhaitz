...
const oak: Species = {
  ...
  traits: { margin: ['lobed'], leaf: ['simple'] },
  distinctiveNotes: '',
}
const privet: Species = {
  commonName: 'Aligustre',
  scientificName: 'Ligustrum',
  traits: { margin: ['entire', 'wavy'], leaf: ['simple'] },
  distinctiveNotes: '',
}
const ash: Species = {
  commonName: 'Fresno',
  scientificName: 'Fraxinus',
  traits: { margin: ['serrate'], leaf: ['compound'] },
  distinctiveNotes: '',
}
...
describe('candidatesFor', () => {
  ...
  it('applies multiple answers cumulatively', () => {
    const r = candidatesFor(all, [
      { traitId: 'leaf', value: 'simple' },
      { traitId: 'margin', value: 'lobed' },
    ])
